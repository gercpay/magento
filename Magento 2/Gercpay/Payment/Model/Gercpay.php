...
namespace Gercpay\Payment\Model;
...
/**
 * Class Gercpay
 * @package Gercpay\Payment\Model
 */
class Gercpay extends \Magento\Payment\Model\Method\AbstractMethod
{
    const METHOD_CODE           = 'gercpay';
    ...
    public function __construct(
        ...
        $writer = new \Zend\Log\Writer\Stream(BP . '/var/log/gercpay.log');
    ...
    public function getSignature($option, $keys)
    {
        ...
        $secret = $this->_scopeConfig->getValue('payment/gercpay/secret_key');
    ...
    {
        return str_contains($this->_scopeConfig->getValue('payment/gercpay/allowed_carrier'), $shippingMethod);
    }
    ...
    {
        return $this->_scopeConfig->getValue('payment/gercpay/request_url') ?: $this->_gateUrl;
    }
    ...
    public function getDataIntegrityCode()
    {
        return $this->_encryptor->decrypt(
            $this->_scopeConfig->getValue('payment/gercpay/secret_key')
        );
    }
    ...
    public function getPostData($orderId)
    {
        ...
        $fields = [
            'operation'    => 'Purchase',
            'merchant_id'  => $this->_scopeConfig->getValue('payment/gercpay/merchant'),
            'order_id'     => $orderId,
            'amount'       => $amount,
            'currency_iso' => $order->getOrderCurrencyCode(),
            'description'  => $description,
            'add_params'   => [],
            'approve_url'  => $this->urlBuilder->getUrl('gercpay/url/gercpaysuccess'),
            ...
            'callback_url' => $this->urlBuilder->getUrl('gercpay/url/gercpayservice'),
            // Statistics.
        ...
        try {
            ...
            if ($response["transactionStatus"] === self::TRANSACTION_APPROVED) {
                ...
                if ($response['type'] === self::RESPONSE_TYPE_PAYMENT) {
                    // Ordinary payment.
                    $status = $this->_scopeConfig->getValue('payment/gercpay/after_pay_status');
                    $order->setState($status)->setStatus($status)->save();
                } elseif ($response['type'] === self::RESPONSE_TYPE_REVERSE) {
                    // Refunded payment.
                    $status = $this->_scopeConfig->getValue('payment/gercpay/after_refund_status');
                    $order->setState($status)->setStatus($status)->save();
                }
    ...
    {
        $merchant = $this->_scopeConfig->getValue('payment/gercpay/merchant');
